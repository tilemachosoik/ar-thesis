...
def check_magnum(conf, *k, **kw):
    ...
    global_includes_check = copy.copy(includes_check)
    global_libs_check = copy.copy(libs_check)
    ...
    if len(global_path) > 0:
        if isinstance(global_path, list):
            ...
            bins_check = [g + '/bin' for g in global_path]
        else:
            includes_check = [global_path + '/include']
            libs_check = [global_path + '/lib']
            bins_check = [global_path + '/bin']
    ...
    try:
        ...
        if 'TARGET_GL' in magnum_config:
            ...
            for gl_file in opengl_files:
                try:
                    opengl_include_dir = get_directory(gl_file, set(includes_check + global_includes_check))
                    gl_not_found = False
                    break
                except:
                    gl_not_found = True
            ...
            if conf.env['DEST_OS'] != 'darwin':
                ...
                opengl_lib_dir = get_directory('libGL.'+suffix, set(libs_check + global_libs_check))
            ...
            if component_type == 'lib':
                ...
                if re.match(pat_app, component):
                    if component == 'GlfwApplication':
                        ...
                        glfw_inc = get_directory('GLFW/glfw3.h', set(includes_check + global_includes_check))
                        ...
                        for lib_glfw in libs_glfw:
                            try:
                                lib_dir = get_directory('lib'+lib_glfw+'.'+suffix, set(libs_check + global_libs_check))
                                glfw_found = True
                                magnum_component_libpaths[component] = magnum_component_libpaths[component] + [lib_dir]
                        ...
                        if conf.env['DEST_OS'] != 'darwin':
                            try:
                                lib_dir = get_directory('libdl.'+suffix, set(libs_check + global_libs_check))
                                magnum_component_libpaths[component] = magnum_component_libpaths[component] + [lib_dir]
                        ...
                        glut_inc = get_directory('GL/freeglut.h', set(includes_check + global_includes_check))
                        ...
                        for lib_glut in libs_glut:
                            try:
                                lib_dir = get_directory('lib'+lib_glut+'.'+suffix, set(libs_check + global_libs_check))
                                glut_found = True
                        ...
                        egl_inc = get_directory('EGL/egl.h', set(includes_check + global_includes_check))
                        ...
                        for lib_egl in libs_egl:
                            try:
                                lib_dir = get_directory('lib'+lib_egl+'.so', set(libs_check + global_libs_check))
                                egl_found = True
                        ...
                        glx_inc = get_directory('GL/glx.h', set(includes_check + global_includes_check))
                        ...
                        for lib_glx in libs_glx:
                            try:
                                lib_dir = get_directory('lib'+lib_glx+'.so', set(libs_check + global_libs_check))
                                glx_found = True
                ...
                if component == 'Audio':
                    ...
                    for inc in includes_audio:
                        try:
                            ...
                            incl_audio = get_directory(inc+'/al.h', set(includes_check + global_includes_check), True)
                            openal_found = True
                    ...
                    for lib_audio in libs_audio:
                        try:
                            lib_dir = get_directory('lib'+lib_audio+'.'+suffix, set(libs_check + global_libs_check))
                            openal_found = True
